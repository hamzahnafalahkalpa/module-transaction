...
use Zahzah\ModuleTransaction\Contracts\Voucher\DateCondition as DateConditionInterface;
...
class DateCondition extends Condition implements DateConditionInterface{
    public function check(Model $voucher_rule,array $attributes): bool{
        ...
        switch (Str::snake($voucher_rule->condition)) {
            case 'in_date_range'   : $result = $now >= $dates[0] && $now <= $dates[1];break;
            case 'less_than_date'  : $result = $now < $dates[0];break;
            case 'after_than_date' : $result = $now > $dates[0];break;
            default :
